...
company="Apna Hotel"
table="Employee Data"
address="Address :- Jalandhar"
contact="Contact information:- 98989898"

class my_cust_PDF(FPDF):
    def header(self):
        self.set_text_color(159,55,98)
        ...
        w = self.get_string_width(table) + 6
        self.set_x((210 - w) / 2)
        self.cell(w, 9, table)
        self.ln(12)
    ...
    def chapter_content(self,data,headings):
        ...
        spacing=1
        col_width = self.w /6.5   # 9 = no of columns +1 to adjust columns to screen
...
#     pdf = my_cust_PDF()
#     # data=[['Room No','ID Number','Name','Amount Paid','Amount Left','Pending Dues'],
#     #       ['Room No','ID Number','Name','Amount Paid','Amount Left','Pending Dues'],
#     #       ['Room No','ID Number','Name','Amount Paid','Amount Left','Pending Dues']]
#
#
#
...
if __name__ == '__main__':
    pdf = my_cust_PDF()
    data=[[23,103,'shivam',4566,200,100]]

    # 'CustomerName', 'CustomerId', 'Gender', 'PhoneNo', 'RoomTypePrefernces'
    headings =['Room No','ID Number','Name','Amount Paid','Amount Left','Pending Dues']
    pdf.print_chapter(data,headings)
